...
public class notifyUser extends TimerTask{
	...
	public void sendReminder(event currentEvent) { //Send Reminder
		try {
			...
			message.setText("Dear User, "
						+ "\n\nToday at "+format.format(currentEvent.getDueDate().getTime())+",You have an event at "+currentEvent.getLocation()
	...
	public void run() {		//Timer Run Method
		...
		while(li.hasNext()) {
			event ce = li.next();
			System.out.println(ce.getDaysUntilDue());
			System.out.println(ce.isFixed());
			if (ce.getDaysUntilDue() < 24 && ce.isFixed() == true) {
				sendReminder(ce);
				li.remove();
